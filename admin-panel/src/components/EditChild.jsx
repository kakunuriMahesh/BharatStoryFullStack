...
const EditChild = ({ stories, updatePart, deletePart }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    ...
    const child = story?.child?.card?.find(c => c.id === childId);
    
    if (child) {
      setValue("titleEn", child.title?.en || "");
      setValue("titleTe", child.title?.te || "");
      setValue("titleHi", child.title?.hi || "");
      setValue("dateEn", child.date?.en || "");
      setValue("dateTe", child.date?.te || "");
      setValue("dateHi", child.date?.hi || "");
      setValue("descriptionEn", child.description?.en || "");
      setValue("descriptionTe", child.description?.te || "");
      setValue("descriptionHi", child.description?.hi || "");
      setValue("timeToReadEn", child.timeToRead?.en || "");
      setValue("timeToReadTe", child.timeToRead?.te || "");
      setValue("timeToReadHi", child.timeToRead?.hi || "");
      setValue("storyTypeEn", child.storyType?.en || "");
      setValue("storyTypeTe", child.storyType?.te || "");
      setValue("storyTypeHi", child.storyType?.hi || "");

      const partFields = child.part?.map((p) => ({
        id: p.id,
  ...
  }, [stories, storyId, childId, setValue]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formDataObj.append("partId", childId);
      formDataObj.append('contentType', 'child');
      ...
      await updatePart(story.name.en, childId, formDataObj);
      navigate('/stories/child');
    } catch (error) {
      console.error('Error updating child content:', error);
      alert('Failed to update child content');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this child content?")) {
      ...
      try {
        ...
        alert('Failed to delete child content');
      } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white shadow rounded mt-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">Edit Child Content</h2>
        <button
          onClick={() => navigate('/stories/child')}
          className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800"
        >
          Back to Child Stories
        </button>
      </div>
      ...
      <div className="mb-4">
        <label className="block font-semibold">Languages for this Child Content:</label>
      ...
      {partLanguages.length > 0 && (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mt-6 flex gap-4">
            ...
            >
              <Upload size={20} /> Update Child Content
            </ButtonLoader>
            <ButtonLoader
              type="button"
            ...
            >
              <Trash size={20} /> Delete Child Content
            </ButtonLoader>
          </div>
        </form>
      )}
